...
def _pipeline(args, dev_cmd, review_cmd, arbiter_cmd,
    ...
    if "review" in completed:
        ...
        if not diff.strip():
            # DEV produced no changes: nothing to review, nothing to merge.
            return _finish(args, workdir, feature, out_dir, state,
    ...
    # an empty findings list only approves when the verdict is also APPROVE.
...
def main(argv=None):
    ...
    # Echo the exact commands (env vars / CLI resolve to shell strings run
    # with shell=True): full visibility beats truncated banners.
    ...
    try:
        ...
        # Reap provider children before _restore touches the worktree.
